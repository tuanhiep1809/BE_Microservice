...
public interface ProductRepository extends JpaRepository<Product,Long> {
    ...
    @Query("SELECT SUM(p.quantitybuy) FROM Product p")
    long sumQuantitybuy();
    
//    @Query(value = "SELECT * FROM Product p WHERE p.quantitybuy > 50 AND p.create_at >= CURRENT_TIMESTAMP - INTERVAL '30' DAY", nativeQuery = true)
//    List<Product> findTopSellingProductsLast30Days();
}
